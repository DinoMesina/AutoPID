/*
   AutoPID_V2 BasicTempControl Example Sketch
...
#define TEMP_READ_DELAY 900 // can only read digital temp sensor every ~750ms

// PID_V2 settings and gains
#define OUTPUT_MIN 0
...
AutoPID_V2 myPID_V2(&temperature, &setPoint, &outputVal, OUTPUT_MIN, OUTPUT_MAX, KP, KI, KD);
...
void setup() {
  ...
  myPID_V2.setBangBang(4);
  // set PID_V2 update interval to 4000ms
  myPID_V2.setTimeStep(4000);
...
void loop() {
  ...
  myPID_V2.run(); // call every loop, updates automatically at certain time interval
  analogWrite(OUTPUT_PIN, outputVal);
  digitalWrite(LED_PIN, myPID_V2.atSetPoint(1)); // light up LED when we're at setpoint +-1 degree
