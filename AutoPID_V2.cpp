  ...
  if (_bangOn && ((*_setpoint - *_input) > _bangOn)) {
    ...
    *_output = _outputMin;
    ...
    if (_dT >= _timeStep) {                               // if long enough, do PID calculations
      ...
      double _deriv = (*_input - _previousInput) / _dT;   // derivative of input, it is not influenced by the setpoint changes  
      double PD = (_error * _Kp) - (_deriv * _Kd);        // PD (-D because is referenced to the input values)
      _integral += ((_error + _previousError) / 2) * _dT; // Riemann sum integral
      _integral = constrain(_integral, ((_outputMin - PD) / _Ki), ((_outputMax - PD) / _Ki)); // limit the integration sum 
